...
func (l *LikeRepo) RecipeExist(ctx context.Context, recipeID int) (bool, error) {
	var id int
	row := l.Pool.QueryRow(ctx, "SELECT id FROM recipes WHERE id=$1", recipeID)
	err := row.Scan(&id)
	...
	if err != nil {
		...
		return false, fmt.Errorf("LikeRepo - RecipeExist - row.Scan: %w", err)
	}
